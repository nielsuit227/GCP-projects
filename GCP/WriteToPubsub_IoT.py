...
def parse_command_line_args():
    ...
    parser.add_argument('--registryId', required=True, help='Cloud IoT Core registry id')
    parser.add_argument('--deviceId', required=True, help='Cloud IoT Core device id')
    parser.add_argument('--privateKeyFile', required=True, help='Path to private key file.')
    parser.add_argument('--algorithm', choices=('RS256', 'ES256'), default='RS256', help='JWT encryption algorithm.')
    parser.add_argument('--cloudRegion', default='europe-west1', help='GCP cloud region')
    parser.add_argument('--caCerts', default='roots.pem', help=('CA root from https://pki.google.com/roots.pem'))
    parser.add_argument('--message', required=True, help='Message to publish')
    parser.add_argument('--baseUrl', default=_BASE_URL, help=('Base URL for the Cloud IoT Core Device Service API'))
...
def main():
    ...
    resp = publish_message(args.message, args.baseUrl, args.projectId, args.cloudRegion, args.registryId, args.deviceId, jwtToken)
